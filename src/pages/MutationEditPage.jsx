  ...
  const validateForm = () => {
    ...
    if (form.relatedToDebt) {
      if (form.type === "keluar" && !form.debtId)
        nextErrors.debtId = "Pilih debt yang ada.";
      if (
        form.type === "masuk" &&
  ...
  if (errorMessage || !mutation) {
    return (
      <PageContainer className="space-y-6">
        <Link
          to="/app/mutations"
          className="inline-flex text-sm font-medium text-slate-600 underline"
        >
        ...
        <button
          ...
          onClick={() =>
            navigate("/app/mutations", {
              ...
              state: {
                message:
                  "Hapus mutasi memang tidak tersedia di UI sesuai aturan produk.",
              },
            })
          }
        ...
        <SectionCard className="space-y-5">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span
                className={`inline-flex rounded-full px-3 py-1 text-sm font-medium ${getMutationTone(mutation.type)}`}
              >
